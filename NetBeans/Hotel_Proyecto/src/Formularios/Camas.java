...
public class Camas extends javax.swing.JPanel {
    ...
    public Camas() {
        initComponents();
        String[] t = {"Codigo Cama","Descripción","Estado","Fecha Registro"};
    ...
    public final void lista_camas(int t,String c) {
        ...
        try {
            ...
            switch(t) {
                case 1:
                    r = getS().executeQuery("SELECT codcama,descripcion,"
                            + "(CASE    WHEN estado=0 THEN 'Activo'"
                            + "         WHEN estado=1 THEN 'Inactivo'"
                            + " END)  AS estado,fechreg FROM cama");
                break;
                case 2:
                    r = getS().executeQuery("SELECT codcama,descripcion,"
                            + "(CASE    WHEN estado=0 THEN 'Activo'"
                            + "         WHEN estado=1 THEN 'Inactivo'"
                            + " END)  AS estado,fechreg FROM cama WHERE codcama like '%"+c+"%'");
                break;
            }
            while(r.next()) {
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Camas", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(153, 153, 153))); // NOI18N
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        switch (jButton1.getText()) {
            ...
            case "Guardar":
                try {
                    getS().executeUpdate("UPDATE cama SET descripcion='"+jTextField1.getText()+"' WHERE codcama="+nc);
                    nc = 0;
                    ti = "";
                    jTextField1.setText("");
                    jButton1.setText("Agregar");
                    ...
                    Logger.getLogger(Camas.class.getName()).log(Level.SEVERE, null, ex);
                }   break;
            case "Agregar":
                if(!jTextField1.getText().equals("")) {
                    try {
                        getS().executeUpdate("INSERT INTO cama(descripcion,estado,fechreg) VALUES('"+jTextField1.getText()+"',0,NOW())");
                        nc = 0;
                        ti = "";
                        jTextField1.setText("");
                        jButton2.setText("Cancelar");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        switch (jButton2.getText()) {
            ...
            case "Inactivo":
                try {
                    getS().executeUpdate("UPDATE cama SET estado=1 WHERE codcama="+nc);
                    reseteo();
                    lista_camas(1,"");
                } catch (SQLException ex) {
                    Logger.getLogger(Camas.class.getName()).log(Level.SEVERE, null, ex);
            ...
            case "Activo":
                try {
                    getS().executeUpdate("UPDATE cama SET estado=0 WHERE codcama="+nc);
                    reseteo();
                    lista_camas(1,"");
                } catch (SQLException ex) {
                    Logger.getLogger(Camas.class.getName()).log(Level.SEVERE, null, ex);
