...
public class Tarifas extends javax.swing.JPanel {
    ...
    public Tarifas() {
        initComponents();
        String[] t = {"Codigo Tarifa","Descripción","Precio","Estado","Fecha Registro"};
    ...
    public final void lista_camas(int t,String c) {
        ...
        try {
            ...
            String consulta ="SELECT codtarifa,descripcion,precio,"
                            + "(CASE    WHEN estado=0 THEN 'Activo'"
                            + "         WHEN estado=1 THEN 'Inactivo'"
                            + " END)  AS estado,fechreg FROM tarifa";
            switch(t) {
                case 1:
                    r = getS().executeQuery(consulta);
                break;
                case 2:
                    r = getS().executeQuery(consulta+" WHERE codtarifa like '%"+c+"%'");
                break;
            }
            while(r.next()) {
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Tarifas", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(153, 153, 153))); // NOI18N
        ...
        jLabel2.setText("Codigo de Tarifa:");
    ...
    private void Lista_CamasMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Lista_CamasMouseReleased
        if(Lista_Camas.getSelectedRow()>=0){
            ...
            pr = Integer.parseInt((String)Lista_Camas.getValueAt(f, 2));
            ti = (String)Lista_Camas.getValueAt(f, 1);
            jTextField1.setEnabled(false);
            precio.setEnabled(false);
            jButton1.setText("Modificar");
            if(Lista_Camas.getValueAt(f, 3).equals("Activo")){
                jButton2.setText("Inactivo");
            } else if(Lista_Camas.getValueAt(f, 3).equals("Inactivo")) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        switch (jButton1.getText()) {
            ...
            case "Guardar":
                try {
                    getS().executeUpdate("UPDATE tarifa SET descripcion='"+jTextField1.getText()+"',precio='"+precio.getText()+"'  WHERE codtarifa="+nc);
                    nc = 0;
                    ti = "";
                    jTextField1.setText("");
                    precio.setText("");
            ...
            case "Agregar":
                if(!jTextField1.getText().equals("")&&!precio.getText().equals("")) {
                    try {
                        getS().executeUpdate("INSERT INTO tarifa(descripcion,precio,estado,fechreg) VALUES('"+jTextField1.getText()+"',"+Double.parseDouble(precio.getText())+",0,NOW())");
                        nc = 0;
                        ti = "";
                        jTextField1.setText("");
                        precio.setText("");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        switch (jButton2.getText()) {
            ...
            case "Inactivo":
                try {
                    getS().executeUpdate("UPDATE tarifa SET estado=1 WHERE codtarifa="+nc);
                    reseteo();
                    lista_camas(1,"");
                } catch (SQLException ex) {
                    Logger.getLogger(Tarifas.class.getName()).log(Level.SEVERE, null, ex);
            ...
            case "Activo":
                try {
                    getS().executeUpdate("UPDATE tarifa SET estado=0 WHERE codtarifa="+nc);
                    reseteo();
                    lista_camas(1,"");
                } catch (SQLException ex) {
                    Logger.getLogger(Tarifas.class.getName()).log(Level.SEVERE, null, ex);
